            ...
            loop {
                ...
                if free_ent > maxcode {
                        ...
                        && block_mode != 0
                    {
                        ...
                        if code >= free_ent {
                            if code > free_ent {
                                if test == 0 && outpos > 0 as libc::c_int {
                                    write_buf(
                                        out,
                                        outbuf.as_mut_ptr() as *mut libc::c_char as voidp,
                                        outpos as libc::c_uint,
                                    );
                                    bytes_out += outpos as off_t;
                                }
                                gzip_error(
                                    if to_stdout != 0 {
                                        b"corrupt input.\0" as *const u8 as *const libc::c_char
                        ...
                        *stackp = finchar as char_type;
                        ...
                        if outpos + i_0 >= 16384 as libc::c_int {
                            loop {
                                if i_0 > 16384 as libc::c_int - outpos {
                                    i_0 = 16384 as libc::c_int - outpos;
                                }
