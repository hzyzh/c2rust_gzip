...
extern "C" {
    ...
    fn lm_init(pack_level: libc::c_int, flags: *mut ush);
...
pub unsafe extern "C" fn zip(
    ...
    *outbuf
    ...
    if outcnt == 16384 as libc::c_int as libc::c_uint {
        flush_outbuf();
    }
    let fresh1 = outcnt;
    outcnt = outcnt.wrapping_add(1);
    ...
    *outbuf.as_mut_ptr().offset(fresh3 as isize) = flags;
    ...
    if outcnt < (16384 as libc::c_int - 2 as libc::c_int) as libc::c_uint {
        let fresh4 = outcnt;
        outcnt = outcnt.wrapping_add(1);
        *outbuf
            .as_mut_ptr()
    ...
    crc = updcrc(0 as *mut uch, 0 as libc::c_int as libc::c_uint);
    bi_init(out);
    ct_init(&mut attr, &mut method);
    lm_init(level, &mut deflate_flags);
    let fresh12 = outcnt;
    outcnt = outcnt.wrapping_add(1);
    ...
    *outbuf.as_mut_ptr().offset(fresh13 as isize) = 0x3 as libc::c_int as uch;
    ...
    if outcnt < (16384 as libc::c_int - 2 as libc::c_int) as libc::c_uint {
        ...
        *outbuf
            ...
            .offset(
                fresh16 as isize,
            ) = (crc & 0xffff as libc::c_int as libc::c_ulong
            & 0xff as libc::c_int as libc::c_ulong) as uch;
        let fresh17 = outcnt;
        outcnt = outcnt.wrapping_add(1);
        *outbuf
            ...
            .offset(
                fresh17 as isize,
            ) = ((crc & 0xffff as libc::c_int as libc::c_ulong) as ush as libc::c_int
            >> 8 as libc::c_int) as uch;
    } else {
        let fresh18 = outcnt;
        outcnt = outcnt.wrapping_add(1);
    ...
    if outcnt < (16384 as libc::c_int - 2 as libc::c_int) as libc::c_uint {
        ...
        *outbuf
            ...
            .offset(
                fresh20 as isize,
            ) = (crc >> 16 as libc::c_int & 0xff as libc::c_int as libc::c_ulong) as uch;
        let fresh21 = outcnt;
        outcnt = outcnt.wrapping_add(1);
        *outbuf
            .as_mut_ptr()
            .offset(
                fresh21 as isize,
            ) = ((crc >> 16 as libc::c_int) as ush as libc::c_int >> 8 as libc::c_int)
            as uch;
    } else {
        let fresh22 = outcnt;
        outcnt = outcnt.wrapping_add(1);
